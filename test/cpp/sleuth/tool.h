...
class ToolArgs {
 public:
  ...
  void Print(std::string s) const { print_fn_(std::move(s)); }

  void SetPrintFn(absl::AnyInvocable<void(std::string) const> print_fn) {
    print_fn_ = std::move(print_fn);
  }

 private:
  explicit ToolArgs(absl::flat_hash_map<std::string, std::string> map)
      : map_(std::move(map)), print_fn_(PrintStdout) {}
  ...
  absl::AnyInvocable<void(std::string) const> print_fn_;
};

using ToolFn = absl::Status (*)(const ToolArgs&);
...
#define SLEUTH_TOOL(name, args_description, description)                  \
  absl::Status name(const grpc_sleuth::ToolArgs& args);                   \
  namespace {                                                             \
  int registration_for_tool_##name =                                      \
      grpc_sleuth::ToolRegistry::Get()->Register(#name, args_description, \
                                                 description, name);      \
  }                                                                       \
  absl::Status name(GRPC_UNUSED const grpc_sleuth::ToolArgs& args)
